...
Schema as of 2026-05-10. Schema version `1.4.0`; lightweight migrations only so far. See `decisions.md` → "Pre-launch schema cleanup" for the in-place V1 migration pattern this project uses pre-launch.

- `Dog`: name, photo (`@Attribute(.externalStorage)`), breed (primary + optional secondary), dateOfBirth, weight, sex, neuterStatus, healthConditions, activityLevel, dailyTargetMinutes, llmRationale (cached), firedMilestones (raw `[String]`), lastRecapSeenWeekStart, archivedAt, createdAt. Relationships: `walkWindows` (cascade), `walks`, `story: Story?` (active book), `completedStories: [Story]?` (cascade-delete archive).
- `Walk`: id, dogIDs (one walk → multiple dogs via `dogs` relationship), startedAt (UTC), durationMinutes, distanceMeters (estimated, from HealthKit), source (passive/manual), notes
- `WalkWindow`: dogID, slot (earlyMorning/lunch/afternoon/evening), enabled
- `Story`: per-dog narrative book. Fields: `genreRaw` (StoryGenre.rawValue), `sceneRaw` (genre-bound scene id), `bible` (rolling LLM-generated state, refreshed at chapter close), `startedAt`, `finishedAt: Date?` (nil = active, non-nil = archived), `title` + `closingLine` (LLM-generated when book ends). Relationship: `chapters: [StoryChapter]?` (cascade).
- `StoryChapter`: 1-indexed within a story. Fields: `index`, `title`, `closingLine`, `closedAt: Date?`, `seenAt: Date?` (the moment the user dismissed the chapter-close overlay), `startedAt`. Relationship: `pages: [StoryPage]?` (cascade), `story: Story?`.
- `StoryPage`: one page of prose + two path teasers. Fields: `index` (1-5 in chapter), `globalIndex` (across the whole book), `prose`, `pathChoiceA` / `pathChoiceB`, `userChoice` (a / b / text / photo / ""), `userText`, `photo` (`@Attribute(.externalStorage)`), `createdAt`. Relationship: `chapter: StoryChapter?`.
...
CloudKit sync enabled at container level (currently `cloudKitDatabase: .none` for pre-launch development; flipped at the end of build). Photos use external storage so they don't count against the 1MB record limit and are more efficient over the wire.
...
- **NotificationService:** permission, scheduling via `UNCalendarNotificationTrigger` (locale-aware), handling of confirmation taps. Enforces the 14-day no-walks safeguard for any dog (no notifications fire if a dog has zero walks in 14 days, regardless of toggles). Permission is asked **contextually** at the walk-window reminder toggle, not at launch (per `decisions.md` → "Notification permission ask via walk-window reminder").
- **LLMService:** calls the Vercel proxy at `/api/dog-voice`. Multi-purpose surface — covers onboarding card, daily Home dog-voice line, walk-complete message, weekly recap narrative, decay messaging, story page generation (`storyPage`), story chapter close + finale (`storyChapterClose` with `isFinale: Bool`), and Insights "Luna says…". 8s timeout for short kinds; 30s for `storyPage` and `storyChapterClose`. Per-call caching by hash of the relevant inputs. Falls back silently to templated copy on network failure (e.g. `StoryService.fallbackPrologue` for first-page failures).
- **StoryService:** orchestrates the per-dog narrative state machine. States: `noStory` / `awaitingFirstWalk` / `pageReady` / `caughtUp` (with `PageLock` for half-target/full-target/dailyCap reasons) / `chapterClosed`. Generation rules: 1 page per walk with 50%/100% milestone gating; 2 pages per local day cap; 5 pages per chapter; `StoryGenre.chaptersPerBook` chapters per book (5 across the board for v1). When the final chapter closes, the book is moved from `dog.story` to `dog.completedStories` and a finale overlay fires. Pure-function `currentState(for:)` over the model graph; LLM-touching paths return `GenerationResult` cases. DEBUG-only helpers (`StoryService+Debug.swift`) for force-finish + seed-completed-book, mirrored to `trot://debug/story/...` deep links.
